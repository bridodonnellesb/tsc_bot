...
MINIMUM_SUPPORTED_AZURE_OPENAI_PREVIEW_API_VERSION = "2024-08-01-preview"
...
@bp.route("/skillset/get_docx_text", methods=["POST"])
async def get_docx_text():
    try:
        request_json = await request.get_json()
        ...
        for item in values:
            url = item["data"]["url"]
            docx_text= extract_text_with_substring(blob_service_client, url)
...
def extract_formulas_with_image_data(blob_service_client, document_analysis_client, image_data, docx_text, identifier):
    ...
    result = analyze_document_with_retry(document_analysis_client, image_bytes)
    ...
    if len(result.pages[0].words)>0:
        ...
        formulas = get_relevant_formula_for_normalised_images(result, identifier)
